...
# Assuming your data frame is named 'combined'
# Assuming your combined dataset is named 'combined'
combined1 <- combined[, c("EVENT_ID", "EPISODE_ID", "STATE", "YEAR", "INJURIES_DIRECT", "INJURIES_INDIRECT", "DEATHS_DIRECT", "DEATHS_INDIRECT", "DAMAGE_PROPERTY", "DAMAGE_CROPS", "FLOOD_CAUSE", "BEGIN_LAT", "BEGIN_LON", "END_LAT", "END_LON", "MONTH_NAME", "BEGIN_DATE_TIME", "END_DATE_TIME","BEGIN_YEARMONTH", "END_YEARMONTH")]
...
quantiles <- quantile(flood_summary$Frequency, probs = c(0.25, 0.5, 0.75))

# Categorize states into frequency levels
flood_summary <- flood_summary %>%
  mutate(Frequency_Level = case_when(
    Frequency > quantiles[3] ~ 'High Frequency',
    Frequency > quantiles[2] ~ 'Moderate Frequency',
    Frequency > quantiles[1] ~ 'Low Frequency',
    TRUE ~ 'Very Low Frequency'
  ))

# View the summary with frequency levels
print(flood_summary)
```
...
flood_cause_stats <- combined1 %>%
  ...
  ungroup() %>%  # Remove grouping before calculating percentages
  mutate(
    Percentage_of_Count = (Count / sum(Count)) * 100,
    Percentage_of_Injuries = (TotalInjuries / sum(TotalInjuries)) * 100,
    Percentage_of_Deaths = (TotalDeaths / sum(TotalDeaths)) * 100,
    Percentage_of_Damage = (TotalDamage / sum(TotalDamage)) * 100
  ) %>%
